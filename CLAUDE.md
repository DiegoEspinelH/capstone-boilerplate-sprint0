...
Pages: `/` · `/auth/signin` · `/auth/signup` · `/team` (post-login landing page, no shell chrome) · `/dashboard` · `/profile` · `/settings` (route groups `(auth)`, `(dashboard)`). Backend: `GET /api/health` (public); everything else under `/api` requires `Authorization: Bearer <ID token>`.
